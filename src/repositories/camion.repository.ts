...
export class CamionRepository {
  ...
  // Busca todos los camiones que están marcados como 'verificado'
  ...
  // Busca un camion en particular por ID
  static async findById(camionId: number): Promise<CamionVerificado | null> {
    try {
    const query = `
      ...
      console.error(`Error al buscar camión ID ${camionId}:`, error);
      throw new Error("No se pudo obtener el camión.");
    }
  }
  ...
  static async findConfiguracionByCamionId(camionId: number): Promise<ConfiguracionCamion | null> {
    try {
      ...
      const query = `
        ...
        WHERE 
          fk_id_camion = $1
          AND es_modificado = false
        ORDER BY id DESC -- Asumimos que el ID más alto es la configuración más reciente
        LIMIT 1;
      `;
  ...
  // Crea un camión (solo puede hacerlo el admin)
  static async create(nuevoCamion: NuevoCamion): Promise<any> {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');

      // 1. Insertar Camion (el controller verifica que sea un admin)
      const camionQuery = `
